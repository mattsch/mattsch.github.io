...
# $Header: /var/cvsroot/gentoo-x86/net-nds/openldap/openldap-2.2.28-r3.ebuild,v 1.5 2005/12/07 03:31:38 dang Exp $
...
make_smbk5pwd() {
	...
	DEFS="${defs}" emake || die "smbk5pwd module compile failed!"
...
install_smbk5pwd() {
	...
	cd ${D}/usr/lib/openldap/openldap
	dosym smbk5pwd.so.0.0.0 /usr/lib/openldap/openldap/smbk5pwd.so.0 
	dosym smbk5pwd.so.0.0.0 /usr/lib/openldap/openldap/smbk5pwd.so
	cd ${S}
}
...
src_unpack() {
	...
	# Password sync overlay module
	EPATCH_OPTS="-p0 -d ${S}" epatch ${FILESDIR}/${PN}-smbk5pwd.patch
...
src_compile() {
	...
	if use kerberos || use samba; then
		make_smbk5pwd
	fi
}
